...
class TestVideo2: UIViewController , AVCaptureFileOutputRecordingDelegate {
    ...
    //最大允许的录制时间（秒）
    let totalSeconds: Float64 = 155.00
    //每秒帧数
    var framesPerSecond:Int32 = 311110
    //剩余时间
    ...
    override func viewDidLoad() {
        ...
        videoLayer.frame = CGRect(x:self.view.bounds.width/4, y:self.view.bounds.height/4,
...
//        self.view.addSubview(progressBar)
    ...
    func startTimer() {
        timer = Timer(timeInterval: remainingTime, target: self,
                      selector: #selector(TestVideo2.timeout), userInfo: nil,
                      repeats:true)
    ...
    func startProgressBarTimer() {
        progressBarTimer = Timer(timeInterval: incInterval, target: self,
                                 selector: #selector(TestVideo2.progress),
